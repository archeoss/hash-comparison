...
inline void
xmsx32_test (const void *key, int len, uint32_t seed, void *out)
{
...
{
  *(uint64_t *)out
      = jody_block_hash ((jodyhash_t *)key, (jodyhash_t)seed, (size_t)len);
}
#endif
...
#include "edonr.h"
inline void
edonr224 (const void *key, int len, uint32_t seed, void *out)
{
  // objsize
...
inline void
edonr256 (const void *key, int len, uint32_t seed, void *out)
{
  // objsize
  struct edonr_ctx ctx;
...
#else
  *(uint64_t *)out
      = komihash ((const uint8_t *)key, (size_t)len, (uint64_t)seed);
#endif
}
...
extern "C"
{
  extern uint64_t wyhash_3_64 (const void *key, int len, uint32_t seed);
  inline void
  wyhash_3_test (const void *key, int len, uint32_t seed, void *out)
  {
    *(uint64_t *)out = wyhash_3_64 (key, len, seed);
  }

  extern uint64_t wyhash_64 (const void *key, int len, uint32_t seed);
  inline void
  wyhash64_test (const void *key, int len, uint32_t seed, void *out)
  {
    *(uint64_t *)out = wyhash_64 (key, len, seed);
  }

  extern uint64_t ahash64 (const void *key, int len, uint32_t seed);
  inline void
  ahash64_test (const void *key, int len, uint32_t seed, void *out)
  {
    *(uint64_t *)out = ahash64 (key, len, seed);
  }
}
