...
fn bench_hash64(c: &mut Criterion) {
    ...
    for input in &PARAMS {
        ...
        let wyhasher_v3 = WyHash::new(SEED, hash_test::final3::make_secret(SEED));
        ...
        //     });
        // });
        group.bench_with_input("wyhash v3", input, move |b, &size| {
            b.iter(|| {
                black_box({
                    let mut hasher = wyhasher_v3.clone();
                    hasher.write(&DATA.as_slice()[..size]);
                    hasher.finish();
                });
            });
